...
class GolfSimulator:
    ...
    def _create_shot_distributions(self):
        """
        Create per–shot-type distributions for:
        - Carry distance (1D Gaussian)
        - Accuracy: (Face-to-Path, Launch Direction) 2D Gaussian
        Accuracy stores means, stds, and the correlation term needed to build a
        full bivariate normal distribution during sampling.
        """
        distributions = {}

        # Loop over all shot types the player has hit
        for shot_type in self.shots_df["Shot Type"].unique():
            shot_data = self.shots_df[self.shots_df["Shot Type"] == shot_type]

            if len(shot_data) < 1:
                # Should never happen because unique() returned it, but safe check
                continue

            # -------------------------------
            # 1) CARRY DISTANCE DISTRIBUTION
            # -------------------------------
            carry_vals = shot_data["Carry (yards)"]
            carry_mean = carry_vals.mean()

            if len(carry_vals) > 1:
                carry_std = carry_vals.std()
            else:
                # fallback: 10% of mean or at least 1 yard
                carry_std = max(1.0, carry_mean * 0.10)

            # -------------------------------
            # 2) 2D ACCURACY DISTRIBUTION
            # -------------------------------
            ftp = shot_data["Face to Path (Deg)"]
            ld  = shot_data["Launch Direction (Deg)"]
            ...
            if len(shot_data) > 1:
                ...
                corr = np.corrcoef(ftp, ld)[0, 1]
                if np.isnan(corr):   # safety guard if weird data
                    corr = 0.0
            else:
                # If only 1 shot exists for this type
                ftp_std = 3.0
                ld_std  = 2.0
                corr    = 0.0
            ...
            distributions[shot_type] = {
                ...
                "accuracy": {
                    "face_to_path_mean": ftp_mean,
                    "face_to_path_std": ftp_std,
                    "launch_direction_mean": ld_mean,
                    "launch_direction_std": ld_std,
                    "corr": corr,            # <--- NEW & IMPORTANT
                },
                "count": len(shot_data),
            }
    ...
    def _sample_accuracy_vector(self, shot_type, current_state):
        """
        Sample a 2D accuracy vector (Face to Path, Launch Direction)
        from a bivariate normal distribution for this shot type.
        """
        if shot_type not in self.shot_distributions:
            # Generic fallback: independent normals
            ftp_mean, ftp_std = 0.0, 3.0
            ld_mean, ld_std   = 0.0, 2.0
            corr = 0.0
        else:
            acc_info = self.shot_distributions[shot_type]['accuracy']
            ftp_mean = acc_info['face_to_path_mean']
            ftp_std  = acc_info['face_to_path_std']
            ld_mean  = acc_info['launch_direction_mean']
            ld_std   = acc_info['launch_direction_std']
            corr     = acc_info.get('corr', 0.0)

        # Build covariance matrix for the 2D Gaussian
        cov_ftp = ftp_std ** 2
        cov_ld  = ld_std ** 2
        cov_off = corr * ftp_std * ld_std
        ...
        mean_vec = np.array([ftp_mean, ld_mean])

        # Sample (FTP, LD) ~ N(mean_vec, cov_matrix)
        ftp_err, ld_err = np.random.multivariate_normal(mean_vec, cov_matrix)

        # Apply lie penalties by inflating the errors
        if current_state == "Rough":
            ftp_err *= (1 + self.rough_penalty)
            ld_err  *= (1 + self.rough_penalty)
        elif current_state == "Bunker":
            ftp_err *= (1 + self.bunker_penalty)
            ld_err  *= (1 + self.bunker_penalty)

        return ftp_err, ld_err

    
    def _determine_shot_accuracy(self, shot_type, current_state):
        """
        Determine landing state based on a 2D Gaussian accuracy model
        in (Face to Path, Launch Direction) space.
        """
        ftp_err, ld_err = self._sample_accuracy_vector(shot_type, current_state)

        # 2D error magnitude (Euclidean norm)
        total_error = np.sqrt(ftp_err ** 2 + ld_err ** 2)

        # Thresholds are in "degrees of combined miss" space
        # You can tweak these based on real data / intuition
        if total_error < 2:
